...
import eu.modelwriter.smtlib.texteditor.Activator;
...
public class Utils {
    ...
    public void runSolver(String solver, IFile file, String text) {
    	...
    	if (!solver.equals("")) try {// FIXME - make "test" a symbolic constant
//    		exec = "" //Preferences.getExec(solver);
     ...
     */
    public void launchJob(String name, final String[] cmd, /*@Nullable*/ final Shell shell) {
    	Job j = new Job("SMT Solver: " + name) {
    		@Override
    		public IStatus run(IProgressMonitor monitor) {
    			boolean c = false;
    			try {
    				int exitCode = 0; // smt.exec(cmd);
     ...
     */
    public void interactiveJob(String name, String[] cmd, /*@Nullable*/ final Shell shell) {
    			try {
    				int exitCode = 0; //smt.exec(cmd);
    				String timestring = "[" + new Date().toString() + "] ";
    ...
    private void addFiles(IResource resource, List<IFile> list) throws CoreException {
    	if (resource instanceof IFile) {
    		if (resource.getName().toString().endsWith(SUFFIX)) {
...
//	    		}
				...
				String fname = dir + File.separator + name + SUFFIX;
    ...
    public void getValue(Shell shell, ISelection selection) {
    	...
    	try {
    		...
    		int e = 0;//saved_smt.execCommand("(get-value (" + text + "))");
    		if (e != 0) {
    			Activator.utils.showMessage(shell,windowHeader,"The selected text is not a valid expression:\n" + text);
    		} else {
    			String response = "";
    			Activator.utils.showMessage(shell,windowHeader,"Value: " + response);
    		}
    	} catch (PluginException e) {
    		Activator.utils.topLevelException(shell,windowHeader,e);
    	}
